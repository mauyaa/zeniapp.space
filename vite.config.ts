...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      react(),
      // Pre-compress build assets once using gzip + brotli.
      ...(isProd
        ? [
            compression({
    ...
    // Optimize dependency pre-bundling to keep cold start fast.
    optimizeDeps: {
      include: [
        'react',
        'react-dom',
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // Manual chunk splitting keeps large vendors in cacheable chunks.
          manualChunks(id) {
            // React core
