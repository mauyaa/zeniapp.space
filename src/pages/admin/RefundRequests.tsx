...
function getUser(r: RefundRequestItem) {
  ...
  if (typeof u === 'object' && u && u !== null && 'name' in u)
    return (
      (u as { name?: string; emailOrPhone?: string }).name ||
      (u as { emailOrPhone?: string }).emailOrPhone ||
      '—'
    );
  return '—';
}
...
export function RefundRequestsPage() {
  ...
  const handleResolve = async () => {
    ...
    try {
      await runWithStepUp(() =>
        resolveRefundRequest(id, { decision, adminNotes: notes || undefined })
      );
      ...
      push({
        title: 'Failed',
        description: e instanceof Error ? e.message : 'Could not resolve',
        tone: 'error',
      });
    } finally {
      setResolvingId(null);
  ...
  return (
    <PageTransition className="space-y-8">
      ...
      <div
        className="flex items-center gap-2 p-1 bg-zinc-50 border border-zinc-200 rounded-xl w-fit"
        role="tablist"
      >
        <button
          type="button"
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          subtitle={
            filter === 'pending'
              ? 'When tenants request refunds they will appear here.'
        ...
        <ul className="space-y-4" role="list">
          {items.map((r) => {
            ...
            return (
              <li
                key={r._id}
                className={cn('bg-white border rounded-2xl p-6 shadow-sm', config.border)}
              >
                  ...
                  {isPending && (
                    <div className="flex flex-col gap-2 flex-shrink-0 lg:w-48">
                      <Button
                        ...
                        size="zeni-sm"
                        onClick={() =>
                          setResolveModal({ id: r._id, decision: 'approved', notes: '' })
                        }
                      ...
                      <Button
                        ...
                        size="zeni-sm"
                        onClick={() =>
                          setResolveModal({ id: r._id, decision: 'rejected', notes: '' })
                        }
                        disabled={resolvingId === r._id}
