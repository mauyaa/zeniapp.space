...
export function LeadsPage() {
  ...
  // Deduplicate by buyer + listing (or support thread) to avoid duplicate cards.
  ...
  const totalLeads = dedupedLeadConversations.length;
  const hotLeads = dedupedLeadConversations.filter((lead) => lead.unreadCount > 0).length;
  const inViewing = dedupedLeadConversations.filter((lead) => lead.leadStage === 'viewing').length;

  const grouped = useMemo(() => {
    return stages.map((stage) => {
      const items = dedupedLeadConversations
        .filter((conv) => conv.leadStage === stage.key)
        .map((conv) => ({
          id: conv.id,
          name: conv.userSnapshot?.name || `Buyer ${conv.userId?.slice(-4) || ''}`.trim(),
          listing: conv.listingSnapshot?.title || 'Unknown property',
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Move buyers through the funnel with fast responses and consistent follow-up. Pending:{' '}
          {totalLeads} leads.
        </p>
      </div>
      ...
      {dedupedLeadConversations.length === 0 ? (
        <EmptyState
          variant="light"
          title="No leads yet"
        ...
        <div className="rounded-sm border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="grid grid-cols-1 gap-0 md:grid-cols-5">
            {grouped.map((col) => (
              <div
                key={col.stage}
                className="border-b md:border-b-0 md:border-r border-gray-200 last:md:border-r-0"
              >
