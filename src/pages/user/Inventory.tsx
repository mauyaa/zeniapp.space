...
import {
  dedupeById,
  dedupeByKey,
  dedupeListingsByContent,
  getListingContentKey,
} from '../../utils/dedupeById';
      ...
      >
        <div className="p-6 border-b border-gray-200 flex-shrink-0">
          <div className="flex justify-between items-center mb-2">
            ...
            <span
              className="bg-gray-100 text-gray-700 px-2.5 py-1 rounded text-sm font-semibold tabular-nums min-w-[2rem] text-center"
              title={total > 0 ? `Showing ${listings.length} of ${total}` : undefined}
            >
          ...
          {loading && listings.length === 0 ? (
            <>
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="bg-white border border-gray-200 rounded-lg p-3 flex gap-4 animate-pulse"
                >
            ...
            listings.map((item, index) => {
              const specs = [
                ...
                item.features.sqm
                  ? `${item.features.sqm} sqm`
                  : item.location?.neighborhood || item.type,
              ]
                .filter(Boolean)
                .join(' • ');
              return (
        ...
        <div className="absolute bottom-12 left-12 right-12 bg-white/90 backdrop-blur p-6 border border-black/5 shadow-xl pointer-events-none">
          <div className="flex justify-between items-end">
            ...
            <div className="flex gap-2">
              <span className="w-8 h-8 border border-black/20 flex items-center justify-center text-xs">
                +
              </span>
              <span className="w-8 h-8 border border-black/20 flex items-center justify-center text-xs">
                −
