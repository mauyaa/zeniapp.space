...
import { KYC_POLICY, KYC_ACCEPTANCE_CRITERIA, KYC_REQUIRED_LABEL } from '../../constants/verification';
...
export function ProfilePage() {
  ...
  const [kycEvidence, setKycEvidence] = useState<{ url: string; note?: string; uploadedAt?: string }[]>([]);
      ...
      >
        <StatsBar
          ...
          items={[
            ...
            { label: 'Last active', value: latestSession?.lastUsedAt ? new Date(latestSession.lastUsedAt).toLocaleDateString() : '--' },
          ]}
        />
      </SectionHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <div className="flex items-center gap-4 mb-6">
             <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-green-500 text-white text-xl font-serif">
                {(user?.name || 'U').charAt(0).toUpperCase()}
              </div>
              <div>
                <h2 className="text-lg font-serif font-semibold text-zinc-900">{user?.name || 'User'}</h2>
                <p className="text-xs text-zinc-500">{user?.email || user?.phone || 'No contact info'}</p>
              </div>
              <div className="ml-auto flex items-center gap-2">
                ...
                <div className="flex rounded-lg border border-zinc-200 p-0.5 bg-zinc-50" role="group" aria-label="Language">
                  <button type="button" onClick={() => setLocale('en')} className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md ${locale === 'en' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}>EN</button>
                  <button type="button" onClick={() => setLocale('sw')} className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md ${locale === 'sw' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}>SW</button>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="bg-zinc-50 rounded-lg px-4 py-3">
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1">Account ID</p>
              ...
              <div className="bg-zinc-50 rounded-lg px-4 py-3">
                ...
                <p className="text-sm font-semibold text-zinc-700 capitalize">{user?.role || 'user'}</p>
              </div>
            </div>
          </div>
          ...
          <div id="kyc" className="bg-white border border-zinc-200 rounded-xl p-6 scroll-mt-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-serif font-semibold text-zinc-900">Verify my identity (KYC)</h2>
            ...
            <div className="mb-4 rounded-lg bg-zinc-50 border border-zinc-200 p-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2">When verification is required</p>
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-3 mb-2">Not required for</p>
            ...
            <div className="mb-4 rounded-lg border border-zinc-200 p-3 bg-white">
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2">Documents we accept</p>
              <ul className="text-xs text-zinc-600 space-y-1">
            ...
            {(kycStatus === 'none' || kycStatus === 'rejected') && (
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  if (!kycFile) {
                    push({ title: 'Select a file', description: 'Upload an ID image', tone: 'error' });
                    return;
                  }
                  const maxSize = 5 * 1024 * 1024; // 5MB
                  if (kycFile.size > maxSize) {
                    push({ title: 'File too large', description: 'KYC documents must be 5MB or smaller. Use a smaller image.', tone: 'error' });
                    return;
                  }
                  if (!/^image\/(jpeg|png|webp|gif)$/i.test(kycFile.type)) {
                    push({ title: 'Invalid file type', description: 'Use JPEG, PNG, WebP or GIF for ID documents.', tone: 'error' });
                    return;
                  }
                  ...
                  try {
                    ...
                    push({ title: 'Submitted', description: 'KYC documents sent for review', tone: 'success' });
                    setKycFile(null);
                    setKycNote('');
              ...
              >
                <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 px-4 py-8 min-h-[120px] text-center text-sm text-zinc-600 hover:border-amber-400 hover:text-amber-700 transition-colors touch-manipulation">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => setKycFile(e.target.files?.[0] || null)} />
                </label>
                <input
                  type="text"
                ...
                />
                <Button type="submit" disabled={kycSubmitting} leftIcon={<FileText className="h-4 w-4" />}>
            ...
            {kycEvidence.length > 0 && (
              <div className="mt-3 pt-3 border-t border-zinc-100">
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-2">Submitted documents</p>
                <div className="flex flex-wrap gap-2">
                  {kycEvidence.map((ev, idx) => (
                    <a key={idx} href={ev.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 text-xs text-zinc-700 hover:border-amber-300">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: 'Saved listings', icon: Bookmark, to: '/app/saved', desc: 'Your saved searches' },
              { label: 'Messages', icon: MessageSquare, to: '/app/messages', desc: 'Agent conversations' },
              { label: 'Payments', icon: Wallet, to: '/pay/login', desc: 'Manage billing' },
              { label: 'Refund requests', icon: Shield, to: '/app/refunds', desc: 'Zeni Shield refunds' },
        ...
        <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-zinc-100">
            ...
            {!loadingSessions && sessions.map((session) => {
              ...
              return (
                <div key={session._id} className="flex items-center justify-between gap-3 px-5 py-4 hover:bg-zinc-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-zinc-500">{session.ip || 'IP unavailable'}</span>
                        <span className="text-zinc-300">·</span>
                        <span className="text-xs text-zinc-500">
                          {session.lastUsedAt ? new Date(session.lastUsedAt).toLocaleString() : 'Unknown'}
                        </span>
                      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-zinc-200 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-serif font-semibold text-zinc-900">Security posture</h2>
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Session management', value: `${sessions.length} active`, ok: sessions.length <= 5 },
                { label: 'Role', value: user?.role || 'user', ok: true },
                { label: 'Last login', value: latestSession?.lastUsedAt ? new Date(latestSession.lastUsedAt).toLocaleString() : 'Unknown', ok: true },
              ].map((item) => (
                <div key={item.label} className="flex items-center justify-between rounded-lg bg-zinc-50 px-4 py-3">
                  <span className="text-sm text-zinc-700">{item.label}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-zinc-900 capitalize">{item.value}</span>
                    <span className={cn(
          ...
          <div className="bg-zinc-50 border border-zinc-100 rounded-xl p-5">
            <p className="text-xs text-zinc-500">
              Need to change your password or enable two-factor authentication? Contact support or use the payment portal security center for payment-related security controls.
            </p>
            <Button
              variant="zeni-secondary"
              size="zeni-sm"
