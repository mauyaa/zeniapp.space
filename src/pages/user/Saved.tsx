...
import {
  setCachedSavedListings,
  getCachedSavedListings,
  cachedListingToProperty,
  uncacheListingCard,
} from '../../lib/savedListingsCache';
  ...
  const handleUnsaveListing = useCallback(async (listingId: string) => {
    try {
      ...
      setSavedListings((prev) =>
        prev.map((l) => (l.id === listingId ? { ...l, saved: false } : l))
      );
    } catch {
      // keep UI as is
    }
      ...
      {tab === 'listings' && (
        <>
          ...
          {savedListingsLoading ? (
            ...
            (() => {
              const quick = quickBarItems.length
                ? quickBarItems
                : (savedListings.slice(0, 12) as CachedListing[]);
              const buckets =
                groupBy === 'city'
                  ? savedListings.reduce<Record<string, (ListingCard | CachedListing)[]>>(
                      (acc, item) => {
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="bg-white border border-zinc-200 rounded-xl p-5">
                  ...
                  <p className="text-xs text-zinc-500">
                    Use Explore to set your preferred location, price, and features.
                  </p>
                </div>
                <div className="bg-white border border-zinc-200 rounded-xl p-5">
                  ...
                  <p className="text-xs text-zinc-500">
                    Hit "Save search" to keep your filters for instant access.
                  </p>
                </div>
                <div className="bg-white border border-zinc-200 rounded-xl p-5">
                  ...
                  <p className="text-xs text-zinc-500">
                    New matches appear in your dashboard automatically.
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="rounded-2xl overflow-hidden border border-gray-200 bg-white hover-lift">
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {itemCount} {itemCount === 1 ? 'search' : 'searches'} •{' '}
                          {formatDateShort(search.createdAt)}
                        </p>
                      </div>
                    </div>
                  </motion.button>
              ...
              <div className="flex items-start gap-4 bg-zinc-50 border border-zinc-100 rounded-xl p-5">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-zinc-800 mb-1">
                    Tip: Save individual listings too
                  </h3>
                  <p className="text-xs text-zinc-500">
                    Tap the heart on any listing card to save it. Your favorites sync across all
                    your devices.
                  </p>
                </div>
              </div>
