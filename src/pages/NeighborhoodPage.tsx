...
const NEIGHBORHOODS: Record<string, { title: string; description: string; faqs: { q: string; a: string }[] }> = {
    kilimani: {
        title: 'Kilimani',
        description:
        ...
        faqs: [
            { q: 'What is the average rent in Kilimani?', a: 'Studios start at KES 25K; 1-bed from KES 35K; 2-bed KES 55K–120K depending on finish and amenities.' },
            { q: 'Is Kilimani safe?', a: 'Yes — Kilimani is considered one of Kenya\'s safer neighbourhoods with active neighbourhood watch schemes.' },
            { q: 'Are pets allowed?', a: 'Many managed properties allow pets; confirm with the agent before listing.' },
        ],
    },
    westlands: {
        title: 'Westlands',
        ...
        faqs: [
            { q: 'Is Westlands good for expats?', a: 'Very popular with expats — proximity to Westgate, ABC Place, and major employers makes it a top pick.' },
            { q: 'What is the average rent?', a: '1-bed from KES 45K; 2-bed from KES 75K for a modern apartment.' },
            { q: 'How is the traffic from Westlands?', a: 'Matatus and Ubers are plentiful; off-peak commute to CBD is under 20 minutes.' },
        ],
    },
    karen: {
        title: 'Karen',
        ...
        faqs: [
            { q: 'Is Karen far from the CBD?', a: 'Yes, about 18km — roughly 30–45 mins depending on traffic. Best suited for those working in Ngong Rd corridor or from home.' },
            { q: 'What are typical rents in Karen?', a: '3-bed houses from KES 120K; townhouses in gated estates from KES 80K.' },
            { q: 'Is there piped water in Karen?', a: 'Most properties rely on boreholes and tanks. Always ask the agent about water reliability.' },
        ],
    },
    lavington: {
        title: 'Lavington',
        ...
        faqs: [
            { q: 'How does Lavington compare to Kilimani?', a: 'Lavington is quieter and more residential; slightly less nightlife but better for families.' },
    ...
    runda: {
        ...
        faqs: [
            { q: 'Can I rent in Runda?', a: 'Yes — most properties are 4–6-bed houses from KES 200K upwards.' },
            { q: 'Is Runda near the CBD?', a: 'About 22km — a 35–50 min drive. Mostly suitable for those working in Muthaiga or Gigiri.' },
        ],
    },
    parklands: {
        title: 'Parklands',
        ...
        faqs: [
            { q: 'Is Parklands good for families?', a: 'Very — excellent schools, parks, and a close-knit community make it ideal for families.' },
...
export function NeighborhoodPage() {
    ...
    useEffect(() => {
        ...
        return () => { document.title = 'Zeni — Where Kenya Lives'; };
    ...
    if (!data) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 px-4 text-center">
                <p className="text-zinc-600">We don't have a guide for <strong>{neighborhood}</strong> yet.</p>
                <Link to="/explore" className="text-sm text-emerald-600 underline">Browse all listings</Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-10 space-y-10">
            {/* Breadcrumb */}
            <nav className="flex items-center gap-2 text-xs font-mono text-zinc-400" aria-label="Breadcrumb">
            ...
            <header>
                <div className="flex items-center gap-2 mb-3">
                    <span className={`px-2 py-1 text-[10px] font-mono font-bold uppercase tracking-widest rounded-md ${purpose === 'buy' ? 'bg-zinc-900 text-white' : 'bg-emerald-600 text-white'}`}>
                        For {purposeLabel}
                    </span>
            ...
            <section>
                ...
                {loading ? (
                                ...
                                <div className="p-3">
                                    ...
                                    <div className="flex items-center justify-between mt-1">
                                        <span className="text-xs font-mono text-zinc-500 flex items-center gap-1">
                                            <BedDouble className="w-3 h-3" />{l.beds ?? '?'} bed
                                        </span>
                    ...
                    <div className="text-center py-16 text-zinc-400 text-sm font-mono">
                        ...
                        <Link to="/explore" className="text-emerald-600 underline">browse all</Link>.
                    </div>
                )}
                {listings.length > 0 && (
