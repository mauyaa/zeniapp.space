...
export function ImageGallery({ images, alt, lqipUrl }: ImageGalleryProps) {
    ...
    return (
        <>
            ...
            {/* Thumbnail strip */}
            {total > 1 && (
                <div className="flex gap-2 mt-3 overflow-x-auto pb-1 snap-x" role="list" aria-label="Image thumbnails">
                    {images.map((src, i) => (
                        <button
                            key={src}
                            onClick={() => setActiveIdx(i)}
                            className={`flex-shrink-0 w-16 h-14 rounded-lg overflow-hidden border-2 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 snap-start ${i === activeIdx ? 'border-emerald-500 ring-2 ring-emerald-200' : 'border-transparent opacity-60 hover:opacity-80'
                                }`}
                            ...
                            <div className="flex gap-2 mt-4 overflow-x-auto pb-2 px-4 max-w-2xl">
                                {images.map((src, i) => (
                                    <button
                                        key={src}
                                        onClick={() => setActiveIdx(i)}
                                        className={`flex-shrink-0 w-14 h-10 rounded-md overflow-hidden border-2 transition-all focus:outline-none ${i === activeIdx ? 'border-emerald-400 opacity-100' : 'border-transparent opacity-40 hover:opacity-70'
                                            }`}
                                    >
