 ...
 * Events: viewing_requested, viewing_fee_paid, refund_requested, refund_resolved, property_paid.
 */

export type AnalyticsEvent =
  | {
      name: 'viewing_requested';
      payload: { listingId: string; viewingId: string; hasFee: boolean };
    }
  ...
  | {
      name: 'property_paid';
      payload: { listingId: string; amount: number; currency: string; purpose?: string };
    };

function sendToGtag(event: AnalyticsEvent) {
  try {
    const gtag = (
      window as unknown as { gtag?: (a: string, b: string, c: Record<string, unknown>) => void }
    ).gtag;
...
function sendToBackend(event: AnalyticsEvent) {
  const endpoint = (import.meta as { env?: { VITE_ANALYTICS_ENDPOINT?: string } }).env
