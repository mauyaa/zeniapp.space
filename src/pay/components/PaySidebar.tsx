...
function ZeniLogo({ className }: { className?: string }) {
  return (
    <div
      className={`flex items-center justify-center rounded-lg bg-emerald-800 ${className ?? 'w-10 h-10'}`}
    >
  ...
  const content = (
    <>
      <div>
        <Link
          to="/pay/dashboard"
          className="px-6 mb-12 flex items-start gap-3 hover:opacity-90 transition-opacity"
        >
