...
async function runWithTransactionFallback<T>(
  ...
  try {
    ...
    logger.info(
      '[pay] Mongo transaction unsupported in this environment, using non-transactional fallback'
    );
    return operation();
  } finally {
...
export async function markTransactionPaidAndNotify(
  ...
  const { txDoc, shouldInvalidateListingSearch } = await runWithTransactionFallback(
    async (session) => {
      ...
      if (txDoc.status !== 'paid') {
        throw Object.assign(new Error('Cannot issue receipt for non-paid transaction'), {
          status: 409,
          code: 'INVALID_STATE',
        });
      }
  ...
  try {
    ...
    const user = await UserModel.findById(txDoc.userId).select('emailOrPhone name');
    const email = user?.emailOrPhone?.includes('@') ? user.emailOrPhone : null;
    if (email) {
      const firstName = (user?.name || '').split(' ')[0] || 'there';
      const purpose = txDoc.purpose || 'your transaction';
      await sendMail(
        email,
        'Payment receipt from ZENI',
        renderBrandEmail({
          title: 'Payment received',
          body: `Hi ${firstName},<br/>We received your payment of <strong>${txDoc.amount} ${txDoc.currency}</strong> for ${purpose}.<br/>Your receipt number is <strong>${receiptNumber}</strong>.`,
          ctaLabel: 'View receipts',
          ctaHref: (process.env.APP_URL || 'http://localhost:5173') + '/payments',
        })
      );
    }
  } catch (err) {
    logger.warn('[pay] receipt email failed', err);
  }
}
...
export async function initiatePortalPayment(
  ...
  if (payload.method === 'mpesa_stk') {
    if (!payload.phone) {
      throw Object.assign(new Error('Phone is required for M-Pesa'), {
        status: 400,
        code: 'PHONE_REQUIRED',
      });
    }
...
export async function handlePortalCallback(payload: MpesaCallbackPayload) {
  ...
  if (nextStatus === 'paid') {
    await markTransactionPaidAndNotify(
      tx,
      payload.receipt || `RCPT-${Date.now()}-${tx.id.slice(-6)}`,
      payload.providerRef,
      {
    ...
    const io = getIO();
    if (io) {
      io.to(`user:${tx.userId.toString()}`).emit('pay:transaction', {
        id: tx.id,
        status: tx.status,
...
export function registerPayGauges() {
  const gauge = getOrCreateGauge('pay_transactions_status', 'Pay portal transactions by status', [
    'status',
  ]);
  PayTransactionModel.aggregate([{ $group: { _id: '$status', count: { $sum: 1 } } }])
    .then((rows) => {
      gauge.reset();
