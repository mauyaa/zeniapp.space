...
export function listAllUsers() {
  return UserModel.find().select('name role status emailOrPhone agentVerification createdAt').sort({ createdAt: -1 });
}

export async function updateUserStatusService(actorId: string, targetUserId: string, status: 'active' | 'suspended' | 'banned') {
  ...
  if (before.role === 'admin' && status !== 'active') {
    const otherAdmins = await UserModel.countDocuments({ role: 'admin', _id: { $ne: targetUserId } });
    if (otherAdmins === 0) {
  ...
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: `user_${status}`, entityType: 'user', entityId: targetUserId, after });
...
export async function deleteUserService(actorId: string, targetUserId: string) {
  ...
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: 'user_delete', entityType: 'user', entityId: targetUserId });
  const io = getIO();
  if (io) {
...
export function listPendingAgents() {
  return UserModel.find({ role: 'agent', agentVerification: 'pending' })
    .select('name email emailOrPhone role agentVerification verificationEvidence earbRegistrationNumber earbVerifiedAt createdAt')
    .lean();
}
...
export async function listModerationQueue(): Promise<ModerationQueueItem[]> {
  ...
  kycUsers.forEach((u: any) => {
    items.push({
      ...
      timestamp: (u.kycSubmittedAt || u.createdAt) ? new Date(u.kycSubmittedAt || u.createdAt).toISOString() : '',
      status: 'Review',
      payload: u
    });
  });
  ...
  businessAgents.forEach((a: any) => {
    items.push({
      ...
      timestamp: (a.businessVerifySubmittedAt || a.createdAt) ? new Date(a.businessVerifySubmittedAt || a.createdAt).toISOString() : '',
      status: 'Review',
      payload: a
    });
  });
...
export async function resolveUserKyc(actorId: string, userId: string, decision: 'approve' | 'reject') {
  ...
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: `user_kyc_${decision}`, entityType: 'user', entityId: userId, after: { kycStatus: status } });
  const io = getIO();
  ...
  await createNotification(userId, {
    ...
    description: decision === 'approve' ? 'Your identity has been verified.' : 'Verification was not approved. You can resubmit documents.'
  });
  triggerAdminDashboard();
  return after;
}

export async function resolveBusinessVerify(actorId: string, agentId: string, decision: 'approve' | 'reject') {
  const status = decision === 'approve' ? 'verified' : 'rejected';
  const after = await UserModel.findByIdAndUpdate(agentId, { businessVerifyStatus: status }, { new: true });
  if (!after || after.role !== 'agent') throw Object.assign(new Error('Agent not found'), { status: 404 });
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: `business_verify_${decision}`, entityType: 'user', entityId: agentId, after: { businessVerifyStatus: status } });
  const io = getIO();
  ...
  await createNotification(agentId, {
    ...
    description: decision === 'approve' ? 'Your business verification is complete.' : 'Business verification was not approved.'
  });
  triggerAdminDashboard();
  return after;
}
...
export async function verifyAgent(actorId: string, agentId: string, decision: 'approve' | 'reject') {
  const after = await UserModel.findByIdAndUpdate(agentId, { agentVerification: decision === 'approve' ? 'verified' : 'rejected' }, { new: true });
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: `agent_${decision}`, entityType: 'user', entityId: agentId, after });
  const io = getIO();
  if (io) {
...
export async function moderateListing(actorId: string, listingId: string, action: string) {
  ...
  await AuditLogModel.create({ actorId, actorRole: 'admin', action: `listing_${action}`, entityType: 'listing', entityId: listingId, after });
  const io = getIO();
  if (io && after) {
...
export async function listNetworkAccessDecisions(limit = 25) {
  ...
  return rows.map((row) => {
    const details = (row.after && typeof row.after === 'object' ? row.after : {}) as Record<string, unknown>;
    return {
      id: String(row._id),
      action: row.action,
...
export async function exportReports(query: any) {
  ...
  const reports = await ReportModel.find(filter).sort({ createdAt: -1 }).limit(Number(limit)).lean();
  const parser = new Parser({
    fields: ['_id', 'category', 'severity', 'status', 'targetType', 'targetId', 'reporterId', 'message', 'createdAt']
  });
...
export async function exportListingsCsv() {
  const listings = await ListingModel.find()
    ...
    .lean();
  const parser = new Parser({
    fields: ['_id', 'title', 'price', 'currency', 'purpose', 'status', 'availabilityStatus', 'verified', 'agentId', 'createdAt']
  });
