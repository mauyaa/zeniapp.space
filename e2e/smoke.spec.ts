...
test.describe('Zeni smoke', () => {
  test('landing page loads and shows app entry', async ({ page }) => {
    ...
    await expect(page.getByText(/Zeni|Explore|Find.*property/i).first()).toBeVisible({ timeout: 15_000 });
  });

  test('login page is reachable and has form', async ({ page }) => {
    await page.goto('/login');
    await expect(page.getByRole('textbox', { name: /email|phone|identifier/i }).first()).toBeVisible({ timeout: 10_000 });
    await expect(page.getByRole('button', { name: /sign in|log in|login/i }).first()).toBeVisible();
  });

  test('explore (app) redirects to login when unauthenticated', async ({ page }) => {
